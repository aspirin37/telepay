...
export default {
  ...
  mounted() {
    if (this.autofocus) setTimeout(() => this.$refs.text.focus());
  },
  watch: {
    value() {
      this.watchValue();
    ...
    addImage(evt) {
      ...
      if (this.images.length + 1 <= this.maxImages) {
        if (image.size / 1024 / 1024 > 1) {
          return this.$notifystr.danger('Ошибка!', 'Размер файла не должен превышать 1мб');
        }
        ...
        this.$notifystr.warning('Внимание', `Можно прикрепить только ${this.maxImages} ${message}`);
      }
    },
    removeImage(index) {
