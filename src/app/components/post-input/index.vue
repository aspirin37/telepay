<template>
    <div class="wrap">
        <div class="textarea-input">
            <textarea class="form-control" rows="4" v-model="text" placeholder="Текст поста..." ref="text"></textarea>
            <div class="textarea-input__icons">
                <label @click="addButton" class="textarea-input__icon">
                    <i class="fa fa-plus-circle" v-tooltip="'Добавить инлайн кнопку'"></i>
                </label>
                <label class="textarea-input__file-label textarea-input__icon">
                    <i class="fa fa-camera text-medium-font" v-tooltip="'Добавить изображение'" aria-hidden="true"></i>
                    <input type="file" accept="image/*" class="textarea-input__file-input" @change="addImage" />
                </label>
                <dropdown :style="{ 'margin-right': '3px' }" class="textarea-input__icon" v-if="$mq != 'sm'">
                    <i slot="trigger" v-tooltip="'Добавить эмоджи'" class="fa fa-smile-o text-medium-font" aria-hidden="true"></i>
                    <div class="textarea-input__emoji" slot="body">
                        <Picker :emoji-size="16" :i18n="i18n" :show-preview="false" :show-search="false" :show-skinTones="false" @select="addEmoji" />
                    </div>
                </dropdown>
            </div>
            <small class="text-muted">{{(text&&text.length)||0}}/{{maxLength}}</small>
        </div>
        <div v-if="images && images.length" class="file-previews">
            <div v-for="(src, i) in images" :key="i" class="file-previews__item" :style="{ 'background-image': `url(${typeof src === 'string'?'/images/posts/'+src:src.decoded})` }">
                <span class="file-previews__remove" @click="removeImage(i)">
                    <i class="fa fa-times" aria-hidden="true"></i>
                </span>
            </div>
        </div>
        <div class="form-row mt-2" :key="btn[0].id" v-for="btn in buttons">
            <div class="col-12 mb-2 mb-lg-0 col-lg">
                <input class="form-control" type="text" v-model="btn[0].text" placeholder="Текст кнопки">
            </div>
            <div class="col-10 mb-2 mb-lg-0 col-lg">
                <input class="form-control" type="text" v-model="btn[0].url" placeholder="Ссылка">
            </div>
            <div class="col-2">
                <button class="btn float-right" @click="removeButton(btn[0].id)">
