  ...
  <div class="preview">
    ...
    <div class="preview__workarea">
      <p class="preview__body">
        ...
        <span class="preview__message">
          <div class="preview__images" v-if="post && post.images && post.images.length">
            <img v-for="(src, i) in post.images" :key="i" class="preview__image" :src="getImageSrc(src)" alt="Image">
          </div>
      ...
      <div class="preview__buttons" v-if="post && post.buttons && post.buttons.length">
        <a target="_blank" rel="nofollow noreferrer noopener" v-for="(btn,i) in post.buttons" :key="i" class="preview__button" :href="`//${btn[0].link}`" v-show="btn[0].text">
