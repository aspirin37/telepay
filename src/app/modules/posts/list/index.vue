        ...
        <div class="flex-table">
          ...
          <div class="body-row" :key="post.postOrderId" v-for="post in posts.in">
            ...
            <div class="col">
              ...
              <a v-else target="_blank" :href="`https://t.me/${post.channelOffer.channel.username}/${post.message_id}`">{{postStatuses[post.status]}}</a>
            </div>
            <div class="col">{{post.sum | centToRub}}</div>
            <div class="col">
              <i class="fa fa-check fa-2x pointer text-success" v-show="post.status === 2" @click="approvePost(post)"></i>
              <i class="fa fa-times fa-2x pointer text-danger" v-show="post.status === 2" @click="declinePost(post)"></i>
